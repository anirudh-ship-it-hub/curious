...
- content: if archetype is causal or system, use kind="graph" — nodes plus explicit edges with a relation label, showing a node affecting MULTIPLE other nodes or a feedback loop wherever the real content actually has that shape (don't force one that isn't there). Otherwise use kind="tree" — a root node with nested children shaped naturally for the archetype (ordered steps, chronological periods, comparison dimensions, hierarchy levels). Editorial quality throughout — like a great explainer, not a chatbot answer or a textbook.
